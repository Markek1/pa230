...
def run(track, load_path, n_eps):
    ...
    return (
        np.mean(eval_env.ep_rews),
        np.median(eval_env.ep_rews),
        np.std(eval_env.ep_rews),
        np.max(eval_env.ep_rews),
        np.min(eval_env.ep_rews),
        interface.TEAM_NAME,
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help="Track id: DQN=0, Policy gradients=1, Cars=2, Cars+Pretraining=3",
    )
    parser.add_argument(
        "--load_path", type=str, required=True, help="Path to the submission folder"
    )
    parser.add_argument(
        "--eps", type=int, default=10, help="Number of evaluation episodes"
    )
    parser.add_argument(
        "--results_path",
        type=str,
